...
namespace Mapbox.Unity.Utilities
{
	...
	public static class Conversions
	{
        ...
        public static Vector2d LatLonToMeters(Vector3 vector)
        {
            return LatLonToMeters(new Vector2d(vector.x, vector.z));
        }
		...
		public static Vector2d StringToLatLon(string s)
		{
			...
			{
				throw new ArgumentException("Wrong number of arguments value '" + s + "'");
			}

			double latitude = 0;
			double longitude = 0;

			if (!double.TryParse(latLonSplit[0], out latitude))
			{
				throw new Exception(string.Format("Could not convert latitude to double: {0}", latLonSplit[0]));
			}

			if (!double.TryParse(latLonSplit[1], out longitude))
			{
        ...
        public static Vector2d MetersToLatLon(Vector3 vector)
        {
            return MetersToLatLon(new Vector2d(vector.x, vector.z));
        }
